#!/usr/bin/env python3
"""Corrected Database Test - All Fields Working"""

import sqlite3
import json
from datetime import datetime

def test_all_database_fields():
    """Test all database fields with comprehensive data"""
    ...
    cursor.execute("""CREATE TABLE payment_details (
        id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_uuid TEXT NOT NULL,
        user_email TEXT NOT NULL, user_name TEXT NOT NULL, plan_type TEXT NOT NULL,
        plan_name TEXT NOT NULL, original_price REAL NOT NULL, discount_percentage REAL,
        discount_amount REAL, final_price REAL NOT NULL, currency TEXT DEFAULT 'USD',
        coupon_code TEXT, payment_method TEXT NOT NULL, payment_status TEXT DEFAULT 'pending',
        transaction_id TEXT, billing_country TEXT, billing_state TEXT, billing_city TEXT,
        billing_address TEXT, billing_postal_code TEXT, company_name TEXT, phone TEXT,
        payment_data TEXT, webhook_data TEXT, payment_date TIMESTAMP, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )""")
    
    cursor.execute("""CREATE TABLE questionnaire_details (
        id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_uuid TEXT NOT NULL,
        user_email TEXT NOT NULL, user_name TEXT NOT NULL, first_name TEXT, last_name TEXT,
        phone TEXT, country TEXT, timezone TEXT, trading_experience TEXT NOT NULL,
        trading_goals TEXT, trading_style TEXT, preferred_markets TEXT, trades_per_day TEXT NOT NULL,
        trading_session TEXT NOT NULL, crypto_assets TEXT, forex_assets TEXT, stock_assets TEXT,
        has_account TEXT NOT NULL, account_equity REAL, prop_firm TEXT, account_type TEXT,
        account_size REAL, account_currency TEXT, broker_name TEXT, broker_platform TEXT,
        risk_percentage REAL NOT NULL, risk_reward_ratio TEXT NOT NULL, custom_risk_reward_ratio REAL,
        ...
        privacy_policy_accepted BOOLEAN, completed_at TIMESTAMP, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )""")
    
    cursor.execute("""CREATE TABLE user_dashboard_data (
        id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_uuid TEXT NOT NULL,
        questionnaire_id TEXT NOT NULL, prop_firm TEXT, account_type TEXT, account_size REAL,
        account_currency TEXT, risk_per_trade REAL, trading_experience TEXT, trading_style TEXT,
        unique_id TEXT, account_balance REAL DEFAULT 0, initial_equity REAL DEFAULT 0,
        current_equity REAL DEFAULT 0, total_pnl REAL DEFAULT 0, daily_pnl REAL DEFAULT 0,
        weekly_pnl REAL DEFAULT 0, monthly_pnl REAL DEFAULT 0, total_trades INTEGER DEFAULT 0,
        winning_trades INTEGER DEFAULT 0, losing_trades INTEGER DEFAULT 0, win_rate REAL DEFAULT 0,
        average_win REAL DEFAULT 0, average_loss REAL DEFAULT 0, profit_factor REAL DEFAULT 0,
        gross_profit REAL DEFAULT 0, gross_loss REAL DEFAULT 0, max_drawdown REAL DEFAULT 0,
        current_drawdown REAL DEFAULT 0, sharpe_ratio REAL, consecutive_wins INTEGER DEFAULT 0,
        consecutive_losses INTEGER DEFAULT 0, max_daily_risk REAL, risk_per_trade_amount REAL,
        ...
        watchlist TEXT, dashboard_layout TEXT, widget_settings TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )""")
    
    print("🎯 COMPREHENSIVE DATABASE FIELD TEST")
    print("=" * 60)
    
    # Test 1: Create users with all 4 plan types
    ...
    print("✅ Payment Details: All fields populated")
    
    # Test 3: Comprehensive Questionnaire Details
    ...
    cursor.execute("""INSERT INTO questionnaire_details (
        ...
        trades_per_day, trading_session, crypto_assets, forex_assets, stock_assets,
        has_account, account_equity, prop_firm, account_type, account_size, account_currency,
        broker_name, broker_platform, risk_percentage, risk_reward_ratio, custom_risk_reward_ratio,
        ...
        ('quest_001', 1, 'uuid-1', 'user1@test.com', 'User1 Test', 'John', 'Doe',
         ...
         '11-20', 'london', 'BTC,ETH,ADA', 'EUR/USD,GBP/USD,AUD/USD', 'AAPL,TSLA,MSFT',
         'yes', 100000.00, 'FTMO', 'live', 100000.00, 'USD', 'Interactive Brokers', 'TWS',
    ...
    print("✅ Questionnaire Details: All 30+ fields populated")
    
    # Test 4: Comprehensive Dashboard Data
    overview_stats = {
    ...
    # Test 5: Verify all data relationships
    ...
    # Test 6: JSON field verification
...
def main():
    ...
    try:
        success = test_all_database_fields()
