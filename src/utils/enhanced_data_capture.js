...
class EnhancedDataCaptureService {
    constructor() {
        this.baseURL = 'http://localhost:5003/api/data-capture';
        this.isEnabled = true;
...
(function() {
    // Override existing savePaymentToDatabase function
    const originalSavePaymentToDatabase = window.savePaymentToDatabase;
    window.savePaymentToDatabase = async function(paymentData) {
        // Call original function first
        let result = null;
        if (originalSavePaymentToDatabase && typeof originalSavePaymentToDatabase === 'function') {
            result = await originalSavePaymentToDatabase(paymentData);
        }
    ...
    // Override existing saveDashboardToDatabase function
    const originalSaveDashboardToDatabase = window.saveDashboardToDatabase;
    window.saveDashboardToDatabase = async function(dashboardData, tradingState, theme) {
        // Call original function first
        let result = null;
        if (originalSaveDashboardToDatabase && typeof originalSaveDashboardToDatabase === 'function') {
            result = await originalSaveDashboardToDatabase(dashboardData, tradingState, theme);
        }
