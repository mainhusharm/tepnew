...
app.post('/api/analyze-symbol', async (req, res) => {
  ...
  try {
    ...
    if (analysisResult.signalType !== 'NEUTRAL') {
      const signalKey = `${analysisResult.symbol}-${analysisResult.entryPrice}-${analysisResult.stopLoss}-${analysisResult.takeProfit}`;
      const now = Date.now();

      if (recentSignals.has(signalKey) && (now - recentSignals.get(signalKey)) < SIGNAL_CACHE_DURATION) {
        console.log(`Duplicate signal detected for ${analysisResult.symbol}. Skipping.`);
      } else {
