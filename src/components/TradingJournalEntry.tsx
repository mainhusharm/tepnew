...
const TradingJournalEntry: React.FC<TradingJournalEntryProps> = ({ 
  ...
  const handleInputChange = (field: string, value: any) => {
    setEntry(prev => {
      const newEntry = { ...prev, [field]: value };
      
      // Auto-calculate fields
      if (field === 'entryPrice' || field === 'exitPrice' || field === 'stopLoss' || field === 'direction' || field === 'positionSize') {
        const entryPrice = newEntry.entryPrice || 0;
        const exitPrice = newEntry.exitPrice || 0;
        const stopLoss = newEntry.stopLoss || 0;
        const direction = newEntry.direction;
        const positionSize = newEntry.positionSize || 0;
        ...
        if (entryPrice && stopLoss) {
          ...
          if (exitPrice) {
            ...
            newEntry.riskRewardRatio = Math.round(riskRewardRatio * 100) / 100;

            const rMultiple = direction === 'long' 
              ? (exitPrice - entryPrice) / (entryPrice - stopLoss)
              : (entryPrice - exitPrice) / (stopLoss - entryPrice);
            newEntry.rMultiple = Math.round(rMultiple * 100) / 100;

            const profitLoss = direction === 'long' 
              ? (exitPrice - entryPrice) * positionSize
              : (entryPrice - exitPrice) * positionSize;
            newEntry.profitLoss = Math.round(profitLoss * 100) / 100;

            const profitLossPercentage = entryPrice > 0 ? (profitLoss / (entryPrice * positionSize)) * 100 : 0;
            newEntry.profitLossPercentage = Math.round(profitLossPercentage * 100) / 100;
          }
        }
      }
      return newEntry;
