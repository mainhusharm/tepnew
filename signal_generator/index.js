...
const cryptoAssets = ['BTCUSD', 'ETHUSD', 'LTCUSD'];
const forexAssets = ['EURUSD', 'GBPUSD', 'USDJPY', 'AUDUSD', 'USDCAD'];
const timeframes = ['1min', '5min', '15min'];

let cryptoInterval = null;
let forexInterval = null;

const generateSignals = async (assets) => {
  const allSignals = [];
...
app.post('/api/signals/crypto/start', (req, res) => {
  if (cryptoInterval) {
    return res.status(400).json({ success: false, message: 'Crypto signals already running.' });
  }
  cryptoInterval = setInterval(() => generateSignals(cryptoAssets), 30000);
  res.json({ success: true, message: 'Crypto signals started.' });
});

app.post('/api/signals/crypto/stop', (req, res) => {
  if (!cryptoInterval) {
    return res.status(400).json({ success: false, message: 'Crypto signals not running.' });
  }
  clearInterval(cryptoInterval);
  cryptoInterval = null;
  res.json({ success: true, message: 'Crypto signals stopped.' });
});

app.post('/api/signals/forex/start', (req, res) => {
  if (forexInterval) {
    return res.status(400).json({ success: false, message: 'Forex signals already running.' });
  }
  forexInterval = setInterval(() => generateSignals(forexAssets), 30000);
  res.json({ success: true, message: 'Forex signals started.' });
});

app.post('/api/signals/forex/stop', (req, res) => {
  if (!forexInterval) {
    return res.status(400).json({ success: false, message: 'Forex signals not running.' });
  }
  clearInterval(forexInterval);
  forexInterval = null;
  res.json({ success: true, message: 'Forex signals stopped.' });
});
