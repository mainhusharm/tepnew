...
class SmartMoneyAnalyzer {
  constructor() {
    ...
    this.binanceServiceURL = 'http://localhost:5010';
    console.log(`🧠 Smart Money Analyzer initialized with Professional Engine and Binance service`);
  }
  ...
  async fetchHistoricalData(symbol, timeframe) {
    try {
      const response = await axios.get(`${this.binanceServiceURL}/api/klines`, {
        params: {
          symbol,
      ...
      console.error(`Error fetching historical data for ${symbol} from Binance service:`, error.message);
      throw new Error(`Failed to fetch historical data for ${symbol} from Binance service.`);
    }
  }
