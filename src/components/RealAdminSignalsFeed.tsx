...
interface RealAdminSignalCardProps {
  signal: Signal;
  isTaken: boolean;
...
const RealAdminSignalCard: React.FC<RealAdminSignalCardProps> = ({ 
  signal, 
  isTaken, 
  onMarkAsTaken, 
  onAddToJournal, 
...
interface RealAdminSignalsFeedProps {
  onMarkAsTaken: (signal: Signal, outcome: TradeOutcome, pnl?: number) => void;
  onAddToJournal: (signal: Signal) => void;
  onChatWithNexus: (signal: Signal) => void;
}

const RealAdminSignalsFeed: React.FC<RealAdminSignalsFeedProps> = ({ 
  onMarkAsTaken, 
  onAddToJournal, 
  onChatWithNexus 
}) => {
  ...
  // Fetch signals from admin dashboard localStorage
  useEffect(() => {
    const fetchAdminSignals = () => {
      try {
        setIsLoading(true);
        setError(null);
        
        console.log('Fetching signals from admin dashboard localStorage...');
        
        // Read signals from admin dashboard localStorage
        const adminSignalsData = localStorage.getItem('admin_signals');
        console.log('Admin signals from localStorage:', adminSignalsData);
        
        if (adminSignalsData) {
          const adminSignals = JSON.parse(adminSignalsData);
          console.log('Parsed admin signals:', adminSignals);
          
          if (Array.isArray(adminSignals) && adminSignals.length > 0) {
            ...
            const transformedSignals = adminSignals.map((signal: any) => ({
              ...
              market: signal.pair?.includes('USDT') || signal.pair?.includes('BTC') || signal.pair?.includes('ETH') ? 'crypto' : 'forex',
            ...
            setConnectionStatus('connected');
            setLastUpdate(new Date());
            console.log('No admin signals found in localStorage');
          }
        } else {
          setSignals([]);
          setConnectionStatus('connected');
          setLastUpdate(new Date());
          console.log('No admin_signals key found in localStorage');
        }
        
      } catch (err) {
        console.error('Error fetching admin signals from localStorage:', err);
    ...
    // Listen for localStorage changes (when admin creates new signals)
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'admin_signals') {
        console.log('Admin signals updated in localStorage, refreshing...');
        fetchAdminSignals();
      }
    };
  ...
  return (
    <div className="real-admin-signals-feed max-w-6xl mx-auto p-6">
      {/* Header with Stats */}
      <div className="mb-8">
...
export default RealAdminSignalsFeed;
