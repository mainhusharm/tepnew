...
def create_or_update_risk_plan():
    ...
    try:
        ...
        message = "Your consent form has been successfully submitted. We will review it shortly."
...
@risk_plan_bp.route('/dashboard-data/<user_email>', methods=['GET', 'OPTIONS'])
def get_dashboard_data(user_email):
    ...
    try:
        ...
        print(f"Dashboard data request for user: {user_email}")
        ...
        risk_plan = RiskPlan.query.filter_by(user_id=user.id).first()
        if not risk_plan:
            # Return a default structure if no risk plan is found
            return jsonify({
                'userProfile': {
                    'propFirm': 'Not Set', 'accountType': 'Not Set', 'accountSize': 'Not Set',
                    'experience': 'Not Set', 'tradesPerDay': 'Not Set', 'riskPerTrade': '1.0%',
                    'riskReward': '1:2.5', 'session': 'Not Set'
                },
                'performance': {
                    'accountBalance': 0, 'winRate': 0, 'totalTrades': 0, 'totalPnL': 0
                },
                'propFirmRules': {},
                'riskProtocol': {},
                'assets': {'crypto': [], 'forex': []}
        ...
        total_pnl = 0.0
        for trade in trades:
            if trade.outcome == 'win' and trade.tp is not None and trade.entry_price is not None:
                total_pnl += abs(trade.tp - trade.entry_price)
            elif trade.outcome == 'loss' and trade.sl is not None and trade.entry_price is not None:
                total_pnl -= abs(trade.entry_price - trade.sl)
        
        # Parse JSON fields safely - handle both JSON and Text columns
        try:
            if risk_plan.crypto_assets:
                if isinstance(risk_plan.crypto_assets, str):
                    crypto_assets = json.loads(risk_plan.crypto_assets)
                else:
                    crypto_assets = risk_plan.crypto_assets if risk_plan.crypto_assets else []
            else:
                crypto_assets = []
                
            if risk_plan.forex_assets:
                if isinstance(risk_plan.forex_assets, str):
                    forex_assets = json.loads(risk_plan.forex_assets)
                else:
                    forex_assets = risk_plan.forex_assets if risk_plan.forex_assets else []
            else:
                forex_assets = []
                
            if risk_plan.prop_firm_compliance:
                if isinstance(risk_plan.prop_firm_compliance, str):
                    prop_firm_compliance = json.loads(risk_plan.prop_firm_compliance)
                else:
                    prop_firm_compliance = risk_plan.prop_firm_compliance if risk_plan.prop_firm_compliance else {}
            else:
        ...
        dashboard_data = {
            'userProfile': {
                ...
                'riskPerTrade': f"{risk_plan.risk_percentage}%" if risk_plan.risk_percentage is not None else '1.0%',
                'riskReward': risk_plan.min_risk_reward or '1:2.5',
        ...
        print(f"Successfully returning dashboard data for {user_email}")
        return jsonify(dashboard_data), 200
        
    except Exception as e:
        import traceback
        error_msg = f"Error fetching dashboard data for {user_email}: {str(e)}"
        print(error_msg)
        print(traceback.format_exc())
        
        return jsonify({'error': 'Dashboard data temporarily unavailable'}), 503
